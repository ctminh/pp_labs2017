...
int main(int argc, const char *argv[]){
    if(argc >= 3){
        ...
        param_t *arg;
        arg = (param_t *) malloc(sizeof(param_t) * num_threads);
        for(i = 0; i < num_threads; i++){
            arg[i].id = i;
            arg[i].dim = dim;
            arg[i].m_a = matrix_a;
            arg[i].m_b = matrix_b;
            arg[i].m_r = result;
            arg[i].rows = &num_rows_per_thread[0];
            
            printf("Creating thread %d\n", i);
            rc = pthread_create(&thread[i], &attr, worker, (void *) &arg[i]);
            if(rc){
        ...
        for(i = 0; i < num_threads; ++i) {
            ...
            printf("Completed joining thread %d having a status of %ld\n", i, (long) &status);
        }
...
void mul_mat(int id, int *rows, int dim, double **matrix_a, double **matrix_b, double **result){
    ...
    finishing_row = starting_row + rows[id];

    //printf("Thread %d:\n \t starting_row = %d - finishing_row = %d\n", id, starting_row, finishing_row);

    for(row = starting_row; row < finishing_row; row++){
        for(col = 0; col < dim; col++){
            sum = 0.0;
            for(k = 0; k < dim; k++){
